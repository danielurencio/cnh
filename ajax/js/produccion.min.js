...
//   if( newCloseExt[1] > this.CloseExt[1] ) console.log("a");
//   console.log(this.yMin,this.yMax);
   if(this.Nacional) {
     this.yMin = newCloseExt[0]// < this.yMin ? newDateExt[0] : this.yMin;
     this.yMax = newCloseExt[1];
     this.xMin = newDateExt[0] //< this.xMin ? newDateExt[0] : this.xMin;
   ...
   innerChart.transition().selectAll(".line")
	.attr("d",function(d) { 
	  ...
	  return activ.erase ? null:line(datasets[id]);  // <-- PROBLEMA
	});

   var newLine = d3.select("#" + activ.field);
if(!newLine.node()) { //console.log(newLine);
